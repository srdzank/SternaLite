...
QPorackiSmetkiVnes::QPorackiSmetkiVnes(QWidget *parent)
	...
	, stat_artikal_call(0)
{
	...
	model2->setHeaderData( 4, Qt::Horizontal, tr("ДДВ (%)"));
	model2->setHeaderData( 5, Qt::Horizontal, tr("Количина"));
	model2->setHeaderData( 6, Qt::Horizontal, tr("Цена"));
	model2->setHeaderData( 7, Qt::Horizontal, tr("Износ"));
	model2->setHeaderData( 8, Qt::Horizontal, tr(""));
	model2->setHeaderData( 9, Qt::Horizontal, tr(""));
	model2->setHeaderData( 10, Qt::Horizontal, tr(""));
...
void QPorackiSmetkiVnes::pressF9()
{
	on_pushButton_4_clicked();
}


void QPorackiSmetkiVnes::pressEscape()
{
    emit closeW();
}
void QPorackiSmetkiVnes::setKomintent(QString text, QString textName)
{
...
void QPorackiSmetkiVnes::setFocusLine()
{
}

void QPorackiSmetkiVnes::setArtikli(QString text, QString textName)
{
...
void QPorackiSmetkiVnes::setSaldo(QString text)
{
// 	ui.lineEdit_5->setText(text);
}

void QPorackiSmetkiVnes::setProdCena(QString text)
{
	ui.lineEdit_3->setText(text);
	m_staraCena = text;
}

void QPorackiSmetkiVnes::setNabCena(QString text)
{
	m_staraNabCena = text;
}

void QPorackiSmetkiVnes::setFocusLine2()
{
	if (stat_artikal_call == 2)
	{
...
void QPorackiSmetkiVnes::on_pushButton_clicked()
{
...
void QPorackiSmetkiVnes::lineEditPressReturn()
{
//    emit scallKomintenti((QWidget*)this, ui.lineEdit->text());
}

void QPorackiSmetkiVnes::on_plineEdit7_Pressed()
{
	CHelperClass hclass(this);
...
void QPorackiSmetkiVnes::on_pushButton_2_clicked()
{
    CHelperClass hclass(this);
...
void QPorackiSmetkiVnes::on_pushButton_9_clicked()
{
	CHelperClass hclass(this);
...
void QPorackiSmetkiVnes::on_pushButton_10_clicked()
{
	on_pushButton_7_clicked();
}

void QPorackiSmetkiVnes::on_pushButton_3_clicked()
{
	CHelperClass hclass(this);
	if (!hclass.isArtikliExists(ui.lineEdit_2->text()))
	{
...
void QPorackiSmetkiVnes::on_pushButton_7_clicked()
{
	CHelperClass hclass(this);
	if (!hclass.isArtikliExists(ui.lineEdit_9->text()))
	{
	...
	model2->setHeaderData( 5, Qt::Horizontal, tr("Количина"));
	item = new QStandardItem(loc.toString(1.f, 'f', 2));
	item->setTextAlignment(Qt::AlignRight);
	item->setEditable(false);
	ui.tableView_2->setRowHeight(row, 18);
	...
	model2->setHeaderData( 6, Qt::Horizontal, tr("Цена"));
	item = new QStandardItem(loc.toString(loc.toFloat(ui.lineEdit_10->text()), 'f', 2));
	item->setTextAlignment(Qt::AlignRight);
	item->setEditable(false);
	ui.tableView_2->setRowHeight(row, 18);
	...
	model2->setHeaderData( 7, Qt::Horizontal, tr("Износ"));
	double iznos = (loc.toFloat("1") * 
		loc.toFloat(ui.lineEdit_10->text()) * 
		(1 - 0.f/100));
	item = new QStandardItem(loc.toString(iznos, 'f', 2));
	item->setTextAlignment(Qt::AlignRight);
	item->setEditable(false);
...
void QPorackiSmetkiVnes::selectionChanged(QModelIndex modelX,QModelIndex modelY)
{
    selRow = modelX.row();
    editorOpen = false;
    emit closeCell();
...
void QPorackiSmetkiVnes::on_pushButton_6_clicked()
{
...
void QPorackiSmetkiVnes::pressReturn()
{
...
void QPorackiSmetkiVnes::on_pushButton_4_clicked()
{
...
void QPorackiSmetkiVnes::on_pushButton_5_clicked()
{
    pressEscape();
}


bool QPorackiSmetkiVnes::existDok(int idDok)
{
    int nCount = 0;
...
bool QPorackiSmetkiVnes::eventFilter(QObject *object, QEvent *event)
{
...
void QPorackiSmetkiVnes::on_plineEdit3_Pressed()
{
    ui.lineEdit_4->setFocus();
	ui.lineEdit_4->selectAll();
}
void QPorackiSmetkiVnes::on_plineEdit4_Pressed()
{
...
void QPorackiSmetkiVnes::on_plineEdit6_Pressed()
{
       
}

void QPorackiSmetkiVnes::openEditor()
{
...
void QPorackiSmetkiVnes::closeEditor()
{
...
QPorackiSmetkiVnes::~QPorackiSmetkiVnes()
{

}

bool QPorackiSmetkiVnes::validateKomintent()
{
 	bool stat = false;
// 	CHelperClass hclass(this);
// 	if (hclass.isKomintentExists(ui.lineEdit->text()))
...
void QPorackiSmetkiVnes::calculateIznosi()
{
	QLocale loc;
	...
	float sumTempIznossoDDV2 = 0.f;

	for (int i = 0; i < rows2; i++)
	{
		sumTempIznossoDDV2 += loc.toFloat(model2->item(i, 8)->text());
	}
	ui.lineEdit_5->setText(loc.toString(chc.rouded(sumTempIznossoDDV2), 'f', 2));
	ui.lineEdit_11->setText(loc.toString(loc.toFloat(ui.lineEdit->text())-loc.toFloat(ui.lineEdit_5->text())));
}

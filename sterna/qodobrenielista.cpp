...
QOdobrenieLista::QOdobrenieLista(QWidget *parent)
...
QOdobrenieLista::~QOdobrenieLista()
{

}

void QOdobrenieLista::setRow(int mrow)
{
	ui.tableView->selectRow(mrow);
}

int QOdobrenieLista::getRow()
{
	return m_row;
}

void QOdobrenieLista::pressEscape()
{
    emit closeW();
}

void QOdobrenieLista::pressReturn()
{
	if (ui.tableView->hasFocus())
	{
		emit sOdobrenieToIzvod(ui.lineEdit_6->text(), ui.lineEdit_8->text(), ui.lineEdit_9->text(), m_selectedText);	
	}
}
void QOdobrenieLista::lista(const QString& nameSearch)
{
    QLocale loc;
    ...
    model->setHeaderData( 1, Qt::Horizontal, tr("Поврат. бр."));
...
void QOdobrenieLista::selectionChanged(QModelIndex modelX, QModelIndex modelY)
{
    int i = modelX.row();
	m_row = modelX.row();
...
void QOdobrenieLista::lista_detail(const QString& nameSearch)
{
    QLocale loc;
    if (nameSearch.length() < 0)
    {
...
void QOdobrenieLista::deleteDok(int idDok)
{
    QSqlQuery query;
    query.clear();
    query.prepare(
...
void QOdobrenieLista::on_pushButton_clicked()
{
    emit procF2();
}
void QOdobrenieLista::on_pushButton_2_clicked()
{
    emit procF3();
}

QStringList QOdobrenieLista::geInfo()
{
    QStringList info;
    info.clear();
...
void QOdobrenieLista::setInitText(QString text)
{
    ui.tableView->setFocus();
}

void QOdobrenieLista::lineEdit7Pressed(const QString & text)
{
    lista(text + "%");
}

...
QPovratnicaKorekcija::QPovratnicaKorekcija(QWidget *parent)
    ...
    , editorOpen(0)
{
	...
	connect(ui.lineEdit_9, SIGNAL(returnPressed()), this, SLOT(on_pushButton_clickedFakturiIzlezni()));
...
void QPovratnicaKorekcija::pressF9()
{
	on_pushButton_4_clicked();
}

void QPovratnicaKorekcija::on_pushButton_clickedFakturiIzlezni()
{
	emit scallFakturi((QWidget*)this, ui.lineEdit->text());
}

QPovratnicaKorekcija::~QPovratnicaKorekcija()
{

}
void QPovratnicaKorekcija::pressEscape()
{
...
void QPovratnicaKorekcija::on_pushButton_clicked()
{
...
void QPovratnicaKorekcija::setFocusLine()
{
    ui.lineEdit->setFocus();
}

void QPovratnicaKorekcija::setFocusLine3()
{
	ui.lineEdit_8->setFocus();
}

void QPovratnicaKorekcija::setKomintent(QString text, QString textName)
{
    ui.lineEdit->setText(textName);
    komintentID = text;
}

void QPovratnicaKorekcija::setArtikli(QString text, QString textName)
{
...
void QPovratnicaKorekcija::setSaldo(QString text)
{
    ui.lineEdit_6->setText(text);
}

void QPovratnicaKorekcija::setFocusLine2()
{
    ui.lineEdit_2->setFocus();
}

void QPovratnicaKorekcija::on_pushButton_2_clicked()
{
    CHelperClass hclass(this);
...
void QPovratnicaKorekcija::on_pushButton_3_clicked()
{
	CHelperClass hclass(this);
	if (!hclass.isArtikliExists(ui.lineEdit_2->text()))
	{
...
void QPovratnicaKorekcija::selectionChanged(QModelIndex modelX,QModelIndex modelY)
{
    selRow = modelX.row();
    editorOpen = false;
    emit closeCell();
...
void QPovratnicaKorekcija::on_pushButton_6_clicked()
{
    model->removeRow(selRow);
}
void QPovratnicaKorekcija::pressReturn()
{
...
void QPovratnicaKorekcija::init(int id)
{
    QLocale loc;
    if(id == -1)
    {
    ...
    QString temp1 = 
        "select dokumenti.id, komintenti.id , komintenti.naziv, datum, dokumenti.rok, dokumenti.zabeleska, dokumenti.user_id "
        "from dokumenti "
    ...
    while (query1.next())
    {
		...
		ui.lineEdit_9->setText(chc.getFakturaBrojOdPovratnica(QString::number(id, 10)));
    }
...
void QPovratnicaKorekcija::on_pushButton_5_clicked()
{
    pressEscape();
}

void QPovratnicaKorekcija::on_pushButton_4_clicked()
{
	...
	try
    {
		...
		query1.clear();
		query1.prepare(
			"delete from povratniciPlakanje where dok_id = :a"
			);
		query1.bindValue(":a", mmid);	
		query1.exec();
		...
		query1.clear();
		query1.prepare(
			"insert into povratniciPlakanje(Dok_ID, IznosVraten, IzleznaFakturaBroj) "
			"values(:a1,:a2,:a3)"
			);
		float m_iznosvraten = 0.f;
		CHelperClass *chc = new CHelperClass(this);
		QString tip = "25";
		chc->createListaIzlezniFakturiIznosi(tip);
		//chc->getFakturaIznosiAll(tip);
		m_iznosvraten = chc->getFakturaVkupno(QString::number(m_id, 10));
		delete chc;

		query1.bindValue(":a1", mmid);	
		query1.bindValue(":a2", QString::number(m_iznosvraten, 'f', 2));	
		query1.bindValue(":a3", ui.lineEdit_9->text());	
		query1.exec();
		QSqlError err1 = query1.lastError();

		
		QMessageBox msgBox;
        msgBox.setText(tr("Повратницата е успешно корегирана."));
        msgBox.setStandardButtons(QMessageBox::Ok);
        msgBox.setDefaultButton(QMessageBox::Ok);
        msgBox.exec();
        pressEscape();
    ...
    catch (...)
    {
        ...
        QMessageBox msgBox;
        msgBox.setText(tr("Повратницата не е корегирана !!!."));
        msgBox.setStandardButtons(QMessageBox::Ok);
        msgBox.setDefaultButton(QMessageBox::Ok);
        msgBox.exec();
    }
}
bool QPovratnicaKorekcija::existDok(int idDok)
{
    int nCount = 0;
...
bool QPovratnicaKorekcija::eventFilter(QObject *object, QEvent *event)
{
...
void QPovratnicaKorekcija::on_plineEdit3_Pressed()
{
    ui.lineEdit_4->setFocus();
}
void QPovratnicaKorekcija::on_plineEdit4_Pressed()
{
...
void QPovratnicaKorekcija::on_plineEdit7_Pressed()
{
	on_pushButton_3_clicked();
}

void QPovratnicaKorekcija::openEditor()
{
...
void QPovratnicaKorekcija::closeEditor()
{
    ui.tableView->closePersistentEditor(indexPrev);
}

bool QPovratnicaKorekcija::validateKomintent()
{
	bool stat = false;
	CHelperClass hclass(this);
	if (hclass.isKomintentExists(ui.lineEdit->text()))
...
void QPovratnicaKorekcija::setFakturi(QStringList& listData)
{
	if(listData.size())
	{
		QStringList rowData = listData.at(0).split(" :: ");
		m_faktura_id = rowData.at(1).trimmed();
		ui.lineEdit_9->setText(m_faktura_id);
		ui.lineEdit_8->setFocus();
	}
}

...
QIzjavaKorekcija::QIzjavaKorekcija(QWidget *parent)
    ...
    , editorOpen(0)
{
	...
	connect(ui.lineEdit_5, SIGNAL(editingFinished()), this, SLOT(lineEdit5EditFinished()));
...
QIzjavaKorekcija::~QIzjavaKorekcija()
{

}

void QIzjavaKorekcija::pressEscape()
{
	emit closeW();
}

void QIzjavaKorekcija::on_pushButton_clicked()
{
...
void QIzjavaKorekcija::setFocusLine()
{
	ui.lineEdit->setFocus();
}
void QIzjavaKorekcija::setKomintent(QString text, QString textName)
{
    ui.lineEdit->setText(textName);
    komintentID = text;
}

void QIzjavaKorekcija::setArtikli(QString text, QString textName)
{
...
void QIzjavaKorekcija::setFocusLine2()
{
	ui.lineEdit_2->setFocus();
}

void QIzjavaKorekcija::on_pushButton_2_clicked()
{
	CHelperClass hclass(this);
...
void QIzjavaKorekcija::on_pushButton_3_clicked()
{
...
void QIzjavaKorekcija::selectionChanged(QModelIndex modelX,QModelIndex modelY)
{
	selRow = modelX.row();
}

void QIzjavaKorekcija::on_pushButton_6_clicked()
{
	model->removeRow(selRow);
}
void QIzjavaKorekcija::pressReturn()
{
...
void QIzjavaKorekcija::init(int id)
{
	QLocale loc;
    if(id == -1)
	{
    ...
    QString temp1 = 
        "select dokumenti.id, komintenti.id , komintenti.naziv, dokumenti.datum "
        "from dokumenti "
        "left join komintenti on dokumenti.komintent_id = komintenti.id "
        "where dokumenti.dok_tip = 21 and dokumenti.dok_id = ";        
...
void QIzjavaKorekcija::on_pushButton_5_clicked()
{
	pressEscape();
}

void QIzjavaKorekcija::on_pushButton_4_clicked()
{
        ...
        QSqlQuery query1;
        query1.clear();
        query1.prepare("update dokumenti set dok_id = :a0, datum=:a1, faktura_id=:a2, mag_id=:a3, komintent_id=:a4 where id=:aid and dok_tip = 21");
        ...
        if (!query1.lastError().isValid())
		{
			query1.clear();
			query1.prepare(
				"delete from magacin where dok_tip = 21 and dok_id = :a"
				);
			query1.bindValue(":a", mmid);	
			query1.exec();
			QString temp = "select max(dok_id) from dokumenti where dok_tip = 21";
			QSqlQuery query;
		...
		{
			QMessageBox msgBox;
			msgBox.setText(tr("Приемницата ne е корегирана!"));
			msgBox.setStandardButtons(QMessageBox::Ok);
			msgBox.setDefaultButton(QMessageBox::Ok);
			msgBox.exec();
		}
...
bool QIzjavaKorekcija::existDok(int idDok)
{
    int nCount = 0;
    QString temp = "select count(dok_id) br from dokumenti where dok_tip = 21 and dok_id =";
...
bool QIzjavaKorekcija::eventFilter(QObject *object, QEvent *event)
{
...
void QIzjavaKorekcija::pressF9()
{
	on_pushButton_4_clicked();
}

void QIzjavaKorekcija::on_plineEdit3_Pressed()
{
    ui.lineEdit_4->setFocus();
}
void QIzjavaKorekcija::on_plineEdit4_Pressed()
{
    on_pushButton_3_clicked();
}
void QIzjavaKorekcija::openEditor()
{
...
void QIzjavaKorekcija::closeEditor()
{
    ui.tableView->closePersistentEditor(indexPrev);
}

void QIzjavaKorekcija::lineEdit5EditFinished()
{
	...
	int nCount = 0;
	QString temp = "select count(dok_id) as br from dokumenti where dok_tip = 21 and dok_id =";
	temp += QString::number(ui.lineEdit_5->text().toInt(), 10);
	QSqlQuery query(temp);
	QString country1 = "";
	int fieldNo = query.record().indexOf("br");
	while (query.next()) 
	...
	if (nCount)
	{
		if(ui.lineEdit_5->text().toInt() != m_id)
		{
			QMessageBox msgBox;
			msgBox.setText(tr("Постои изјава со тој број\n Одбери друг број!"));
			msgBox.setStandardButtons(QMessageBox::Ok);
			msgBox.setDefaultButton(QMessageBox::Ok);
			msgBox.exec();
			ui.lineEdit_5->setFocus();
...
bool QIzjavaKorekcija::validateKomintent()
{
	bool stat = false;
	CHelperClass hclass(this);
	if (hclass.isKomintentExists(ui.lineEdit->text()))

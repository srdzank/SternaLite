...
void QKasaKorekcija::setSaldo(QString text)
{
...
void QKasaKorekcija::init(int id)
{
    ...
    QString temp1 = 
        "select dokumenti.id, komintenti.id , komintenti.naziv, datum "
        "from dokumenti "
        "left join komintenti on dokumenti.komintent_id = komintenti.id "
        "where dokumenti.dok_tip = 45 and dokumenti.dok_id = ";        
...
void QKasaKorekcija::on_pushButton_4_clicked()
{
    ...
    QSqlQuery query1;
    query1.clear();
    query1.prepare("update dokumenti set dok_id = :a0, datum=:a1, faktura_id=:a2, mag_id=:a3, komintent_id=:a4 where id=:aid and dok_tip = 45");
    ...
    query1.clear();
    query1.prepare(
        "delete from magacin where dok_tip = 45 and dok_id = :a"
        );
...
bool QKasaKorekcija::existDok(int idDok)
{
    int nCount = 0;
...
bool QKasaKorekcija::eventFilter(QObject *object, QEvent *event)
{
...
void QKasaKorekcija::on_plineEdit3_Pressed()
{
    ui.lineEdit_4->setFocus();
}
void QKasaKorekcija::on_plineEdit4_Pressed()
{
...
void QKasaKorekcija::on_plineEdit7_Pressed()
{
	on_pushButton_3_clicked();
}

void QKasaKorekcija::openEditor()
{
...
void QKasaKorekcija::closeEditor()
{
    ui.tableView->closePersistentEditor(indexPrev);
}

bool QKasaKorekcija::validateKomintent()
{
	bool stat = false;
	CHelperClass hclass(this);
	if (hclass.isKomintentExists(ui.lineEdit->text()))

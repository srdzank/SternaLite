...
void QKalkulacijaKorekcija::on_pushButton_4_clicked()
{
		...
		QMessageBox msgBox;
		msgBox.setText(tr("ПРИЕМНИЦАТА ке биде КОРЕГИРАНА"));
		msgBox.setInformativeText(tr("Одбери OK за да потврдиш."));
		msgBox.setStandardButtons(QMessageBox::Ok | QMessageBox::Cancel);
		msgBox.setDefaultButton(QMessageBox::Ok);
		int ret = msgBox.exec();
		...
		if (sqlStat)
		{
			db.commit();
			QMessageBox msgBox;
			msgBox.setText(tr("Приемница - Калкулацијата е успешно корегирана."));
			msgBox.setStandardButtons(QMessageBox::Ok);
			msgBox.setDefaultButton(QMessageBox::Ok);
			msgBox.exec();
			pressEscape();
		...
		else
		{
			db.rollback();
			QMessageBox msgBox;
			msgBox.setText(tr("Приемница -Калкулацијата НЕ Е корегирана !!!."));
			msgBox.setStandardButtons(QMessageBox::Ok);
			msgBox.setDefaultButton(QMessageBox::Ok);
			msgBox.exec();
		}
...
void QKalkulacijaKorekcija::setPriemnicaID(QString text, QString textName)
{
	...
	model->setHeaderData( 7, Qt::Horizontal, tr("Е.Наб.Цена "));
	model->setHeaderData( 8, Qt::Horizontal, tr("Е.Трошоци "));
